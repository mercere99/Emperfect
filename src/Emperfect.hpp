...
private:
  ...
  // Take an input line and convert any "CHECK" macro into "CHECK_emperfect"
  std::string ApplyCheck(const std::string & line) {
    return line;
  }
  ...
  void GenerateTestCPP(Testcase & test) {
    ...
    if (test.code_filename.size()) {
      emp::notify::TestError(test.code.size(),
        "Test case ", test.id, " has both a code filename and in-place code provided.");
    ...
    // Add testing functions (line spacing works after output)
    cpp_file
      << "#define CHECK_emperfect(LHS, COMP, RHS) {              \\\n"
      << "  auto lhs = LHS;                                      \\\n"
      << "  auto rhs = RHS;                                      \\\n"
      << "  bool success = (lhs COMP rhs);                       \\\n"
      << "  _emperfect_passed &= success;                        \\\n"
      << "  _emperfect_out << _emperfect_check_id << \"\\n\"        \\\n"
      << "      << __LINE__ << \"\\n\"                              \\\n"
      << "      <<  (success ? \" PASSED\\n\" : \" FAILED\\n\")        \\\n"
      << "      << #LHS << \"\\n\" << #COMP << \"\\n\" << #RHS << \"\\n\" \\\n"
      << "      << lhs << \"\\n\" << rhs << \"\\n\";                   \\\n"
    ...
    for (const auto & x : test.code) {
      cpp_file << ApplyCheck(ApplyVars(x)) << "\n";
    }
    ...
    cpp_file
      << "  _emperfect_out << \"SCORE \" << (_emperfect_passed ? "
          << test.points << " : 0) << \"\\n\";\n"
      << "}\n\n"
      << "// Build a test runner that executed before main."
